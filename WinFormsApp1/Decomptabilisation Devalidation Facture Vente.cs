...
{
    public partial class Decomptabilisation_Devalidation_Facture_Vente : Form
    {
        private SqlConnection connection; // Ajoutez ce champ pour stocker la connexion

        public Decomptabilisation_Devalidation_Facture_Vente(SqlConnection connection)
        {
            InitializeComponent();
        ...
        private void Decomptabilisation_Devalidation_Facture_Vente_Load(object sender, EventArgs e)
        {
            FillDataGridView();
            ...
            try
            {
                if (connection != null && connection.State == ConnectionState.Open)
                {
                    ...
                    string query = "SELECT F_DOCENTETE.DO_Piece, F_COMPTET.CT_Intitule AS Client, F_DOCENTETE.DO_Date, " +
                                   "CASE F_DOCENTETE.DO_Valide WHEN 1 THEN 'Validé' ELSE 'Non Validé' END AS Validation, " +
                                   ...
                                   "WHERE F_DOCENTETE.DO_Type = 7 and DO_Valide = 1";
                    ...
                    table.Columns["Validation"].ColumnName = "Validation"; // Renommer la colonne "Validation"
        ...
        private void Executer_btn_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (connection != null)
                {
                    using (SqlCommand command = new SqlCommand("SAGE_VENTE_DOCOM_DEVALI_FACTURE", connection))
                    {
